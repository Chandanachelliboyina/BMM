...
import { apiCheckout, clearToken } from "@/lib/api";
...
function LogoutAttendancePage() {
  ...
  const submitLogout = async () => {
    if (!employee || !selfieBlob) { toast.error("Please capture your logout selfie"); return; }
  ...
  const skip = async () => { await signOut(); };
  ...
  return (
    <div className="min-h-screen bg-secondary/30">
      <header className="h-14 border-b bg-background/80 backdrop-blur flex items-center gap-3 px-4 sticky top-0 z-30">
        ...
        <Button variant="ghost" size="sm" className="ml-auto" onClick={() => navigate({ to: "/dashboard" })}>Cancel</Button>
      </header>
      ...
      <main className="max-w-3xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        ...
        <Card className="p-5 shadow-card">
          ...
          <p className="text-sm text-muted-foreground mb-4">Take a selfie to record your logout time. It will be compared with your profile photo.</p>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="aspect-square rounded-lg overflow-hidden bg-muted border-2 border-primary/20">
                {photoUrl ? <img src={photoUrl} alt={employee.full_name} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-xs text-muted-foreground">No photo</div>}
              </div>
            </div>
            <div>
              ...
              <div className="aspect-square rounded-lg overflow-hidden bg-black/90 relative">
                {selfieUrl ? <img src={selfieUrl} alt="Selfie" className="w-full h-full object-cover" /> : <video ref={videoRef} className="w-full h-full object-cover scale-x-[-1]" muted playsInline />}
                {!cameraOn && !selfieUrl && (
                  <div className="absolute inset-0 flex items-center justify-center bg-black/40">
                    <Button size="sm" onClick={startCamera} className="bg-white text-black hover:bg-white/90"><Camera className="w-3.5 h-3.5 mr-1.5" /> Start</Button>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="mt-4 flex gap-2">
            {!selfieUrl && cameraOn && <Button onClick={capture} className="flex-1 bg-gradient-primary"><Camera className="w-4 h-4 mr-2" /> Capture selfie</Button>}
            {selfieUrl && <Button variant="outline" onClick={retake} className="flex-1"><RefreshCw className="w-4 h-4 mr-2" /> Retake</Button>}
          </div>
        </Card>
        ...
        <Card className="p-5 shadow-card">
          ...
          {location ? <GoogleMapEmbed latitude={location.lat} longitude={location.lng} /> : (
            <div className="h-[240px] rounded-lg border bg-muted flex items-center justify-center text-sm text-muted-foreground">{locError ?? "Fetching your location…"}</div>
          )}
          <div className="mt-4 flex gap-2 justify-end">
            <Button variant="outline" size="sm" onClick={captureLocation}><RefreshCw className="w-3.5 h-3.5 mr-1.5" /> Refresh location</Button>
          </div>
        </Card>
        ...
        <div className="flex justify-between gap-3">
          <Button variant="ghost" onClick={skip} disabled={submitting || verifying || !isAfter6PM}>Skip & sign out</Button>
          <Button onClick={submitLogout} disabled={submitting || verifying || !selfieBlob || !location || !isAfter6PM} className="h-12 px-8 bg-gradient-primary shadow-elegant">
